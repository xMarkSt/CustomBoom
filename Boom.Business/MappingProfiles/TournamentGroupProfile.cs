...
public class TournamentGroupProfile : Profile
{
    public TournamentGroupProfile()
    {
        ...
        CreateMap<LevelTarget, LevelTargetDto>() // todo: see php version
            ...
            .ForMember(dest => dest.Version, opt => opt.MapFrom(src => src.Level.Online ? src.Id : src.Level.Version)) // Use the level target id here to ensure an entry is create in the player's ZLEVEL database for each unique goal
            .ForMember(dest => dest.Target, opt => opt.MapFrom(src => src.Level.Online ? GetOnlineTarget() : string.Empty))  //todo: when online json encode! See php. Otherwise empty.
            ...
            .ForMember(dest => dest.BgName, opt => opt.MapFrom(src => src.Level.Background.BgName)) // todo not working?
            ;
    }
